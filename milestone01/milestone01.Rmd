...
library(corrplot) # used to make correllogram plot
...
q<-df%>%
  mutate(district = factor(district, levels = unique(factor_district$district))) %>% #factor district by descending mean price
  ggplot(aes(district, price)) +
  geom_violin(stat = "ydensity") +
  scale_y_log10() +  # change to log10 scale 
  ylab("Price (€)") +
  xlab("District") +
