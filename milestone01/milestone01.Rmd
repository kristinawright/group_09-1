...
knitr::opts_chunk$set(echo = TRUE,
                      fig.path = paste(here::here("milestone01", "milestone01_files"), "/", sep=''))
library(knitr)
library(tidyverse)
...
A kernel density plot is presented for listing prices of all listings (on the left) and listings excluding the top `r 100 - p.lvl*100`% (on the right). We see that a few outliers extremely skews the density of all listings far to the right. Using the `R` functions, `mean()`, `quantile()`, and `max()`, respectively, we see that the mean listing price for all listings is `r mean(df$price)`, the 3rd quartile for all listings is `r quantile(df$price, probs=0.75)`, while the maximum listing price of all listings is `r max(df$price)`. 

As a result, we exclude the top `r 100 - p.lvl*100`% of listings. The maximum allowed listing price is `r qtile`. The resulting filtered tibble has `r nrow(df) - nrow(df %>% filter(price <= quantile(df$price, probs=p.lvl)))`.

`r summary(df$price)`

```{r price-density, fig.width=8, fig.height=4, fig.cap=paste("Plot (a) presents the Kernel density of listing prices for Barcelona; Plot (b) presents the Kernel density of listing prices for Barcelona where the price per night is less than", qtile, "euros")}
p1 <- df %>% 
  ggplot(aes(x=price)) + 
  geom_density() +
  theme_bw() +
